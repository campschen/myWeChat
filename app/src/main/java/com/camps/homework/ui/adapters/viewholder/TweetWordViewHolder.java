 ...
 * 仅仅包含文字的推文视图
 */
public class TweetWordViewHolder extends TGRecyclerViewHolder<TweetWordBean> implements OnItemClickPopupMenuListener {
    ...
    protected int getLayoutId() {
        return R.layout.item_recycler_firend_circle_only_word;
    }

    @Override
    public void fillData(TweetWordBean itemData, int position, int viewType) {
        //推文内容
    ...
    //设置推文
    private void setContent(TweetWordBean itemData, int position){
    ...
    public void onItemClickTranslation(int position) {
        TweetWordBean tweetWordBean = (TweetWordBean) getAdapter().getItem(position);
        tweetWordBean.setTranslationState(TranslationState.START);
        updateTransView(position, TranslationState.START, null, true);

        TimerUtils.timerTranslation(() -> {
            ((TweetWordBean) getAdapter().getItem(position)).setTranslationState(TranslationState.END);
            updateTransView(position, TranslationState.END, ((TweetWordBean) getAdapter().getItem(position)).getContentSpan(), true);
        });
